...
function finishQuiz() {
    ...
    }).catch(err => ZeroModal.alert("Interupsi basis data saat sinkronisasi EXP: " + err.message));
}
...
window.resetLeaderboard = function() {
    ZeroModal.confirm("OTORISASI ADMIN DIBUTUHKAN: Apakah Anda yakin mengeksekusi penghapusan absolut pada tabel indeks prestasi mingguan?", function(res) {
        if(res) {
            db.collection("leaderboard").get().then(snapshot => {
                if(snapshot.empty) { ZeroModal.alert("Sistem merespons: Indeks tabel telah dikosongkan."); return; }
                let batch = db.batch(); snapshot.forEach(doc => { batch.delete(doc.ref); });
                batch.commit().then(() => { ZeroModal.alert("Operasi berhasil. Tabel peringkat global telah diatur ulang."); loadLeaderboard(); }).catch(err => ZeroModal.alert("Kegagalan operasi batch: " + err.message));
            });
        }
    });
}