...
// KAMUS 10 BAHASA LENGKAP TANPA DI POTONG
const tr = {
    id: { 
    ...
    jp: { login_btn: "ログイン", nav_home: "ホーム", nav_gauss: "ガウス", nav_cramer: "クラメル", nav_addsub: "加減算", nav_trans: "転置", nav_rank: "ランク", nav_det: "行列式", nav_cofactor: "余因子", nav_inv: "逆行列", nav_chat: "グループ", nav_quiz: "テスト", nav_profile: "プロフ", nav_materi: "ライブラリ", nav_gemini: "✨ アシスタント", nav_helpdesk: "🎧 ヘルプ" },
    zh: { login_btn: "登录", nav_home: "主页", nav_gauss: "高斯", nav_cramer: "克莱姆", nav_addsub: "加减", nav_trans: "转置", nav_rank: "秩", nav_det: "行列式", nav_cofactor: "余子式", nav_inv: "逆矩阵", nav_chat: "聊天", nav_quiz: "测验", nav_profile: "资料", nav_materi: "库", nav_gemini: "✨ 助手", nav_helpdesk: "🎧 帮助" },
    ru: { login_btn: "Войти", nav_home: "Главная", nav_gauss: "Гаусс", nav_cramer: "Крамер", nav_addsub: "Сложение", nav_trans: "Транспонирование", nav_rank: "Ранг", nav_det: "Определитель", nav_cofactor: "Кофактор", nav_inv: "Обратная", nav_chat: "Группы", nav_quiz: "Тест", nav_profile: "Профиль", nav_materi: "Библиотека", nav_gemini: "✨ Помощник", nav_helpdesk: "🎧 Помощь" },
    es: { login_btn: "Entrar", nav_home: "Inicio", nav_gauss: "Gauss", nav_cramer: "Cramer", nav_addsub: "Suma/Resta", nav_trans: "Traspuesta", nav_rank: "Rango", nav_det: "Determinante", nav_cofactor: "Cofactor", nav_inv: "Inversa", nav_chat: "Grupos", nav_quiz: "Prueba", nav_profile: "Perfil", nav_materi: "Biblioteca", nav_gemini: "✨ Asistente", nav_helpdesk: "🎧 Ayuda" },
    fr: { login_btn: "Connexion", nav_home: "Accueil", nav_gauss: "Gauss", nav_cramer: "Cramer", nav_addsub: "Add/Sous", nav_trans: "Transposée", nav_rank: "Rang", nav_det: "Déterminant", nav_cofactor: "Cofacteur", nav_inv: "Inverse", nav_chat: "Groupes", nav_quiz: "Test", nav_profile: "Profil", nav_materi: "Bibliothèque", nav_gemini: "✨ Assistant", nav_helpdesk: "🎧 Aide" },
    de: { login_btn: "Einloggen", nav_home: "Start", nav_gauss: "Gauß", nav_cramer: "Cramer", nav_addsub: "Add/Sub", nav_trans: "Transponieren", nav_rank: "Rang", nav_det: "Determinante", nav_cofactor: "Kofaktor", nav_inv: "Inverse", nav_chat: "Gruppen", nav_quiz: "Test", nav_profile: "Profil", nav_materi: "Bibliothek", nav_gemini: "✨ Assistent", nav_helpdesk: "🎧 Hilfe" },
...
function navigate(m) {
    ...
    if(m === 'home') {
        c.innerHTML = `
            <div style="text-align:center; padding: 20px 0; animation: fadeIn 0.8s;">
                <h2 style="font-size:3.5em; font-weight:300; margin-bottom:10px; border:none; color:var(--text-primary); letter-spacing:-1px;">Zero<b style="font-weight:800; color:var(--brand-main);">Matriks</b></h2>
    ...
    if(isSq) { 
        ...
        let v = cols - 1; 
        html += `<span style="font-weight:600;">${t('ui_var')}</span><input type="number" id="dim-n" value="${v}" min="1" max="100" style="width:80px; text-align:center;"><button class="primary-btn" onclick="buildGrid('cr')">${t('ui_set')}</button>`; 
    } else { 
    ...
    if(isSq || isCr) { 
        document.getElementById('dim-n').addEventListener('input', function() { 
            let d = document.getElementById('dim-n-disp'); 
...
function buildGrid(type) {
    ...
    if(type === 'sq') { 
        rows = cols = parseInt(document.getElementById('dim-n').value); 
    } else if(type === 'cr') { 
        rows = parseInt(document.getElementById('dim-n').value); cols = rows + 1; 
    } else { 
        rows = parseInt(document.getElementById('dim-row').value); cols = parseInt(document.getElementById('dim-col').value); 
    }
...
function solve(op) {
    ...
    if (op === 'add' || op === 'sub') { 
        ...
        if(isZero) { 
            ...
            if(showSteps) { 
                htmlOut += `<div class="result-box" style="text-align:left;"><span style="color:var(--text-primary); font-weight:700;">${t('step_det_tri')}</span><br><br><span style="color:var(--text-secondary);">${t('step_det_form')}</span><br><span style="color:var(--text-secondary);">Det = ${detSign < 0 ? '(-1)' : '1'} * (${diags.join(' * ')})</span><br><hr style="border-color:var(--border-subtle); margin:20px 0;">Determinan Akhir = <b style="color:var(--brand-main); font-size:1.5em;">${fN(finalDet)}</b></div>`; 
            } else { 
