window.ZeroModal = {
    ...
    init: function() {
        ...
        this.btnOk.onclick = () => { this.close(); if(this.callback) { let val = this.input.style.display !== 'none' ? this.input.value : true; this.callback(val); } };
        this.btnCancel.onclick = () => { this.close(); if(this.callback) this.callback(null); };
    },
    show: function(type, msg, cb) {
        if(!this.overlay) this.init();
        this.callback = cb; this.desc.innerHTML = msg; this.input.value = '';
        if(type === 'alert') { this.title.innerText = 'Informasi Sistem'; this.icon.innerText = '💬'; this.input.style.display = 'none'; this.btnCancel.style.display = 'none'; this.btnOk.innerText = 'Tutup'; this.btnOk.className = 'primary-btn'; } 
        else if(type === 'confirm') { this.title.innerText = 'Konfirmasi Otorisasi'; this.icon.innerText = '⚠️'; this.input.style.display = 'none'; this.btnCancel.style.display = 'block'; this.btnOk.innerText = 'Lanjutkan'; this.btnOk.className = 'danger-btn'; } 
        else if(type === 'prompt') { this.title.innerText = 'Input Sistem'; this.icon.innerText = '✍️'; this.input.style.display = 'block'; this.btnCancel.style.display = 'block'; this.btnOk.innerText = 'Kirim'; this.btnOk.className = 'primary-btn'; setTimeout(() => this.input.focus(), 100); }
        this.overlay.style.display = 'flex'; setTimeout(() => this.overlay.classList.add('show'), 10);
    },
    close: function() { this.overlay.classList.remove('show'); setTimeout(() => this.overlay.style.display = 'none', 300); },
    alert: function(msg, cb) { this.show('alert', msg, cb); }, confirm: function(msg, cb) { this.show('confirm', msg, cb); }, prompt: function(msg, cb) { this.show('prompt', msg, cb); }
};

const firebaseConfig = { apiKey: "AIzaSyCFwwLiL4FBgAQ13OHVnWVGwxsApcDSR7U", authDomain: "zeromatriks-f3062.firebaseapp.com", projectId: "zeromatriks-f3062", storageBucket: "zeromatriks-f3062.firebasestorage.app", messagingSenderId: "379008700427", appId: "1:379008700427:web:988a7fa91f3367a123770e", measurementId: "G-7E7D2CLSNX" };
firebase.initializeApp(firebaseConfig);
const auth = firebase.auth();
const db = firebase.firestore();

let currentUserData = { name: "Pengguna", role: "Mahasiswa", shortId: "00000", photoBase64: "", level: 1, exp: 0 };
let lang = 'id'; let mode = 'home'; let rows = 2, cols = 3;

// KEMBALI MENGGUNAKAN 10 BAHASA SECARA UTUH
const tr = {
    id: { 
    ...
    en: { 
        login_desc: "Linear Algebra Integrated System", login_user: "Email Address...", login_pass: "Password...", login_btn: "System Login", welcome: "Session:", logout: "Terminate Session", login_toggle_reg: "Register New Entity", login_toggle_log: "Already Registered? Login", btn_register: "Create Account", nav_home: "Main Dashboard", nav_gauss: "Gauss-Jordan", nav_cramer: "Cramer's Rule", nav_addsub: "Basic Ops (+/-)", nav_trans: "Transpose", nav_rank: "Matrix Rank", nav_det: "Determinant", nav_cofactor: "Cofactor", nav_inv: "Inverse Matrix", nav_chat: "Discussion Groups", nav_quiz: "Try Out & Ranking", nav_profile: "Entity Settings", nav_materi: "Reference Library", nav_gemini: "✨ Virtual Assistant", nav_helpdesk: "🎧 Helpdesk"
    },
...
function changeLanguage() { 
    lang = document.getElementById('languageSelect').value; document.getElementById('login-lang').value = lang;
    document.querySelectorAll('.sidebar a').forEach(e => { let k = e.getAttribute('data-lang'); if(t(k) !== k) e.innerText = t(k); }); 
    ...
    document.getElementById('btn_logout').innerText = t('logout'); navigate(mode); 
}
function changeLangFromLogin() { document.getElementById('languageSelect').value = document.getElementById('login-lang').value; lang = document.getElementById('languageSelect').value; changeLanguage(); }

function navigate(m) {
    mode = m; document.querySelectorAll('.sidebar a').forEach(e => e.classList.remove('active')); 
    ...
    let isSq = (m === 'det' || m === 'inv' || m === 'cofactor'); let isCr = (m === 'cramer'); let isDbl = (m === 'addsub');
    ...
    if(isSq) { html += `<span style="font-weight:600;">${t('ui_size')}</span><input type="number" id="dim-n" value="${rows}" min="1" max="100" style="width:80px; text-align:center;"><span>x</span><span id="dim-n-disp" style="font-weight:600;">${rows}</span><button class="primary-btn" onclick="buildGrid('sq')">${t('ui_set')}</button>`; } 
    else if(isCr) { let v = cols - 1; html += `<span style="font-weight:600;">${t('ui_var')}</span><input type="number" id="dim-n" value="${v}" min="1" max="100" style="width:80px; text-align:center;"><button class="primary-btn" onclick="buildGrid('cr')">${t('ui_set')}</button>`; } 
    else { html += `<span style="font-weight:600;">${t('ui_row')}</span><input type="number" id="dim-row" value="${rows}" min="1" max="100" style="width:80px; text-align:center;"><span style="font-weight:600; margin-left:15px;">${t('ui_col')}</span><input type="number" id="dim-col" value="${cols}" min="1" max="100" style="width:80px; text-align:center;"><button class="primary-btn" onclick="buildGrid('${isDbl?'db':'sg'}')">${t('ui_set')}</button>`; }
...
window.submitHelpdesk = function() {
    ...
    db.collection("helpdesk_tickets").add({
        uid: auth.currentUser.uid, name: currentUserData.name,
        ...
        document.getElementById('hd-subject').value = ''; document.getElementById('hd-desc').value = '';
    }).catch(err => ZeroModal.alert("Kegagalan transmisi: " + err.message));
}

// === FUNGSI MATRIKS ALJABAR KEMBALI DIURAIKAN (TIDAK DI-MINIFY) ===
...
// LOGIKA MATEMATIKA YANG SUDAH DIURAIKAN
function solve(op) {
    ...
    if (op === 'add' || op === 'sub') { 
        ...
        let m = cln(A); if(showSteps) htmlOut += rT(m, t('step_init')); 
        ...
        for(let rt=0; rt<r; rt++) { 
            ...
            if(Math.abs(p) > 1e-10) { 
                ...
                if(showSteps && Math.abs(p - 1) > 1e-10) htmlOut += rT(cln(m), t('step_div').replace('{r}', rt+1).replace('{p}', fN(p))); 
            } 
            let eli = false; 
            for(let k=0; k<r; k++) { 
                if(k !== rt) { 
        ...
        for(let i=0; i<rows; i++) { 
            let allZeroA = true; 
            for(let j=0; j<cols-1; j++) { if (Math.abs(m[i][j]) > 1e-10) { allZeroA = false; break; } } 
        ...
        let m = cln(A); let n = rows; if(showSteps) htmlOut += rT(m, t('step_init')); 
            ...
            if(Math.abs(pivot) < 1e-10) { 
                ...
                for(let k=i+1; k<n; k++) { 
                    if(Math.abs(m[k][i]) > 1e-10) { 
                        ...
                        if(showSteps) htmlOut += rT(cln(m), t('step_swap_det').replace('{r1}', i+1).replace('{r2}', k+1)); break; 
                    } 
                } 
        ...
        for(let i=0; i<rows; i++) { 
            ...
            for(let j=0; j<cols; j++) { 
                ...
                let dM = cD(mr); if(Math.abs(dM) < 1e-10) dM = 0; 
                let sg = ((i+j)%2 === 0) ? 1 : -1; 
                let cf = sg * dM; 
                let ds = ""; 
                ...
                if(showSteps) { 
                    if(mr.length === 2) { let a = mr[0][0], b = mr[0][1], cv = mr[1][0], d = mr[1][1]; ds = `(${fN(a)} &times; ${fN(d)}) - (${fN(b)} &times; ${fN(cv)})`; } 
                    else if(mr.length === 1) { ds = `${fN(mr[0][0])}`; } 
                    let mh = `<div style="overflow-x:auto;"><table class="matrix-table" style="font-size:0.9em; margin:10px 0; box-shadow:none;"><tbody>`; 
                    mr.forEach(rr => { mh += `<tr>`; rr.forEach(v => mh += `<td>${fN(v)}</td>`); mh += `</tr>`; }); 
                    mh += `</tbody></table></div>`; 
        ...
        for(let r=0; r<rows; r++) { 
            ...
            if(Math.abs(p - 1) > 1e-10) { 
                ...
                if(showSteps) htmlOut += rT(cln(ag), t('step_div_p').replace('{p}', fN(p)), true, rows); 
            } 
            let el = false; 
            ...
            if(showSteps && el) htmlOut += rT(cln(ag), t('step_elim_p'), true, rows); lead = l++; 
        } 
        if(!ps) { 
