...
window.sendResetLink = function() {
    ...
    if(email === "") {
        ZeroModal.alert("Sistem menolak: Alamat surel tidak boleh kosong.");
        return;
    }
    auth.sendPasswordResetEmail(email).then(() => {
        ZeroModal.alert("Tautan reset kata sandi telah dikirim ke surel Anda. Silakan periksa kotak masuk (inbox) atau folder spam.");
        toggleAuth('login');
        document.getElementById('reset-username').value = '';
    }).catch((error) => {
        ZeroModal.alert("Sistem gagal memproses permintaan: " + error.message);
    });
};
...
function doRegister() {
    ...
    auth.createUserWithEmailAndPassword(email, p1)
        .then((userCred) => {
            ZeroModal.alert("Registrasi entitas berhasil diselesaikan. Menginisialisasi sesi...");
...
function doLogin() {
    ...
    if(email === "") return;
    auth.signInWithEmailAndPassword(email, p).catch((error) => ZeroModal.alert("Otentikasi gagal. Kredensial tidak diakui sistem."));
}

function doLogout() { auth.signOut().catch((error) => ZeroModal.alert("Terminasi sesi gagal: " + error.message)); }
        ...
        if (user) {
            ...
            db.collection("users").doc(user.uid).get().then(doc => {
                if(doc.exists) {
                    ...
                    currentUserData = { 
                        name: user.email.split('@')[0], shortId: newId, role: pendingRegistrationRole || "Mahasiswa", 
                        age: pendingRegistrationAge || null, photoBase64: "", level: 1, exp: 0 
                    };
                    db.collection("users").doc(user.uid).set(currentUserData).then(() => {
                        pendingRegistrationRole = ""; pendingRegistrationAge = null;
                        updateHeaderProfile(); changeLanguage(); navigate('home');
                    });
                }
            }).catch(err => { ZeroModal.alert("Kesalahan Sistem: Tidak dapat memuat basis data profil."); });
        } else {
        ...
        <div style="display:flex; gap:30px; flex-wrap:wrap; align-items:flex-start;">
            <div class="data-card" style="flex:2; min-width:300px; text-align:center;">
                ...
                <div style="margin:30px 0; padding:20px; background:var(--bg-surface); border:1px solid var(--border-subtle); border-radius:12px; display:flex; justify-content:space-around;">
                    ...
                    <div><span style="color:var(--text-secondary); font-size:0.85em; text-transform:uppercase; letter-spacing:1px;">Demografi</span><br><b style="color:var(--accent-success); font-size:1.15em;">${ageDisplay}</b></div>
...
function updateProfileName() {
    ...
    if(newName.length < 6 || newName.length > 15) { return ZeroModal.alert("Sistem menolak: Dimensi identitas harus berkisar 6 hingga 15 karakter."); }
    db.collection("users").doc(auth.currentUser.uid).set({ name: newName }, { merge: true }).then(() => {
        currentUserData.name = newName; updateHeaderProfile(); ZeroModal.alert("Pembaruan parameter identitas dikonfirmasi."); navigate('profile'); 
    ...
    reader.onload = function(e) {
        ...
        img.onload = function() {
            ...
            db.collection("users").doc(auth.currentUser.uid).set({ photoBase64: base64 }, { merge: true }).then(() => {
                currentUserData.photoBase64 = base64; updateHeaderProfile(); ZeroModal.alert("Sinkronisasi visual entitas dikonfirmasi.");
