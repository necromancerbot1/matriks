...
window.resetPassword = function() {
    let email = prompt("Masukkan alamat surel (email) akun Anda untuk mereset kata sandi:");
    if(email && email.trim() !== "") {
        auth.sendPasswordResetEmail(email.trim()).then(() => {
            alert("Tautan reset kata sandi telah dikirim ke surel Anda. Silakan periksa kotak masuk (inbox) atau folder spam.");
        }).catch((error) => {
            alert("Sistem gagal memproses permintaan: " + error.message);
        });
    }
};
...
function doRegister() {
    ...
    if(email === "" || p1 === "" || ageInput === "") { alert("Format isian tidak valid. Mohon lengkapi seluruh parameter, termasuk usia."); return; }
    if(isNaN(ageInput) || parseInt(ageInput) < 10) { alert("Sistem menolak: Parameter usia tidak valid atau di bawah batas minimum."); return; }
