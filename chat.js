...
function acceptFriend(senderUid, senderName, senderShortId) {
    ...
    db.collection("users").doc(myUid).collection("friend_requests").doc(senderUid).delete().then(() => { ZeroModal.alert("Otorisasi selesai. Koneksi jaringan disetujui."); loadFriends(); });
}
...
function createNewGroup() {
    let name = document.getElementById('new-group-name').value.trim(); if(!name) return ZeroModal.alert("Sistem menolak: Parameter nama ruang tidak terdefinisi.");
...
window.deleteGroup = function(groupId, groupName) {
    ZeroModal.confirm(`OTORISASI ADMIN DIBUTUHKAN: Konfirmasi penghapusan absolut untuk ruang kelas [${groupName}] dari server pusat?`, function(res) {
        if(res) {
